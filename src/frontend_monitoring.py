...
with st.spinner(text="Fetching model predictions and actual values from the store"):
    ...
    from_date = pd.Timestamp(current_date - timedelta(days=14))
...
with st.spinner(text="Plotting aggregate MAE hour-by-hour"):
    ...
    mae_per_hour = (
        monitoring_df
        .groupby('pickup_hour')
        .apply(lambda g: mean_absolute_error(g['actuals_rides'], g['predicted_demand']))
        .reset_index()
...
with st.spinner(text="Plotting MAE hour-by-hour for top locations"):
    ...
    top_locations_by_demand = (
        monitoring_df
        .groupby('pickup_location')['actuals_rides']
        .sum()
        .sort_values(ascending=False)
        .reset_index()
        .head(10)['pickup_location']
    )

    for location_id in top_locations_by_demand:
        
        mae_per_hour = (
            monitoring_df[monitoring_df.pickup_location == location_id]
            .groupby('pickup_hour')
            .apply(lambda g: mean_absolute_error(g['actuals_rides'], g['predicted_demand']))
            .reset_index()
            .rename(columns={0: 'mae'})
        ...
        st.plotly_chart(fig, theme="streamlit", use_container_width=True, width=1000, key=f"plot_{location_id}")
